...
function Menu() {
  ...
  const joinQueue = async () => {
    try {
      const res = await axios.post(`${process.env.REACT_APP_API_BASE_URL}/joinqueue`, {}, {
        withCredentials: true,
      });
  
      if (res.data.success) {
        console.log('Joined queue successfully');
        // redirect the user to the lobby page or perform some other action
      } else {
  ...
  return (
    <div>
      <h1>Menu</h1>
      <button onClick={joinQueue}>Multiplayer</button>
