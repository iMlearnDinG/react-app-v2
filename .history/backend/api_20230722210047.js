...
app.post('/joinQueue', async (req, res) => {
  // You should include some authentication check here

  // Find a lobby with 'waiting' status
  let lobby = await Lobby.findOne({ status: 'waiting' });

  if (!lobby) {
    // No lobby is waiting, so create a new one
    lobby = new Lobby();
  }
  ...
  if (lobby.players.length >= 2) {
    // Two or more players in the lobby, so start a new game
    const game = new Game({
      players: lobby.players,
      // ... set other game fields ...
    });
    await game.save();
  ...
  // Send back the lobby information
